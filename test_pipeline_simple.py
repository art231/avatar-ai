#!/usr/bin/env python3
"""
Упрощенный тестовый пайплайн для Дня 4.
...
def test_xtts_service_simple():
    ...
    try:
        ...
        if health_response.status_code == 200:
            ...
            print(f"✓ XTTS Service здоров: {health_data.get('status')}")
...
def test_lipsync_service_simple():
    ...
    try:
        ...
        if health_response.status_code == 200:
            health_data = health_response.json()
            print(f"✓ Lipsync Service здоров: {health_data.get('status')}")
            
            # Проверка доступности процессора
            service_health = health_data.get("details", {}).get("service_health", {})
            if service_health.get("processor_available"):
                print("✓ Процессор липсинка доступен")
                return True
            else:
                print(f"⚠ Процессор липсинка не доступен: {service_health.get('error', 'Неизвестная ошибка')}")
...
def test_xtts_synthesis():
    ...
    try:
        ...
        print(f"✗ Ошибка при тестировании синтеза речи: {e}")
        import traceback
        traceback.print_exc()
        return False, None
    finally:
        ...
        if test_audio_path.exists():
            ...
            print(f"✓ Удален временный файл: {test_audio_path}")

def main():
    """Основная функция тестирования."""
    print("=" * 60)
    print("УПРОЩЕННОЕ ТЕСТИРОВАНИЕ ПАЙПЛАЙНА - ДЕНЬ 4")
    print("=" * 60)
    ...
    if not lipsync_ok:
        print("\n⚠ Lipsync Service имеет проблемы, но продолжаем тестирование")
    
    # Шаг 3: Тестирование синтеза речи
    synthesis_ok, audio_path = test_xtts_synthesis()
    if not synthesis_ok:
        print("\n✗ Пайплайн прерван: не удалось синтезировать речь")
        return False
    ...
    print("=" * 60)
    print(f"✓ XTTS Service: {'РАБОТАЕТ' if xtts_ok else 'НЕ РАБОТАЕТ'}")
    print(f"✓ Lipsync Service: {'РАБОТАЕТ' if lipsync_ok else 'ИМЕЕТ ПРОБЛЕМЫ'}")
    print(f"✓ Синтез речи: {'УСПЕШЕН' if synthesis_ok else 'НЕ УДАЛСЯ'}")
    print(f"✓ Сгенерированное аудио: {audio_path}")
    print(f"✓ Общее время выполнения: {total_time:.2f} секунд")
    
    if xtts_ok and synthesis_ok:
        print("\n✅ ОСНОВНОЙ ФУНКЦИОНАЛ ДНЯ 4 РАБОТАЕТ!")
        ...
        print("\n❌ ОСНОВНОЙ ФУНКЦИОНАЛ ДНЯ 4 НЕ РАБОТАЕТ")
        return False
...
if __name__ == "__main__":
    try:
        ...
        if success:
            print("\n" + "=" * 60)
            print("✅ День 4 завершен успешно!")
            print("=" * 60)
            sys.exit(0)
        else:
            print("\n" + "=" * 60)
            print("❌ День 4 не завершен: есть проблемы с пайплайном")
            print("=" * 60)
            sys.exit(1)
